...
The third-party libraries used by Equinox are:

- Z3 for SMT solving needs (e.g. congruence closure);
- Kissat for SAT needs (e.g. for selecting the best rewritten program);
- UTHash for e-graphs, comes with several useful libraries: UTString, UTArray, UTStack, UTStringBuffer, all of which we use in Equinox;
- SFSexp: S-Expression parsing, for parsing rewrite specs (which can be optionally provided through them);
- JSON and YAML libraries, for data exchange needs;
- jemalloc: allocation;
- SymEngine: for simplification of arithmetic expressions;
